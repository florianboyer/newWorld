CREATE TABLE `variete`(`idVarietee` INTEGER,`nomVarietee` VARCHAR(30),`descriptionVarietee` VARCHAR(200),`photo` VARCHAR(300),`valider` BOOL,`dureeDeVie` INTEGER,primary key(`idVarietee`));
...
CREATE TABLE `produitVariete`(`idVarietee` INTEGER NOT NULL,`idProduit` INTEGER NOT NULL, foreign key (`idVarietee`) references variete(`idVarietee`), foreign key (`idProduit`) references produit(`idProduit`),primary key(`idVarietee`,`idProduit`));
